...
- история ревизий упражнений для create/update/delete/batch changes;
- revision viewer с grouped diff, pretty JSON и readable baseline snapshots;
...
История изменений хранится в `exercise_revisions` как append-only audit/read layer: каждая ревизия имеет `version`, `source`, полный `snapshot`, `changed_fields`, опциональные `actor_label`, `batch_id` и `summary`. Runtime actions не выполняют DDL; схему и совместимость со старым форматом применяют только служебные scripts.
