...
- `/api/exercise-events` - SSE-поток обновлений упражнений.
...
ЕГЭ 13/15, blitz и уже отрендеренные задания в чате сохраняют live refresh: если формулировка, payload, answer или explanation изменились в админке, UI может подтянуть свежую версию из БД без пересборки приложения и без получения нового упражнения. Основной канал - PostgreSQL `LISTEN/NOTIFY` -> `/api/exercise-events` SSE -> browser update hub; fallback polling остаётся страховкой. Для quick/qseed-карточек refresh пересобирает все локально открытые карточки, построенные из изменённого упражнения.
...
- история ревизий упражнений для create/update/delete/batch/restore changes;
- revision viewer с grouped diff, readable baseline snapshots, restore и удалением ручных ревизий;
- best-effort запись ревизий после основной записи упражнения: сбой истории не валит save/create/delete;
- compact history для undo-like правок: если следующий save полностью возвращает snapshot к предпредыдущей версии, последняя ручная/autosave ревизия схлопывается вместо записи шумной пары;
...
- переключение между упражнениями не пишет в БД: отличающаяся форма сохраняется как local draft, чтобы простой клик по списку не менял `updatedAt` и не переставлял карточки;
- draft recovery;
- preview;
- manual refresh;
...
Контракт обновлений: `updatedAt` считается версией упражнения. Главная и quick-модалки точечно обновляют уже отрендеренные карточки по версии через SSE/local events/fallback polling, а админка отправляет известную версию при save, чтобы не затирать изменения из другой вкладки.

История изменений хранится в `exercise_revisions` как audit/read layer: каждая ревизия имеет `version`, `source`, полный `snapshot`, `changed_fields`, опциональные `actor_label`, `batch_id` и `summary`. Restore создаёт новую ревизию с source `restore`, а не переписывает прошлое; delete доступен для ручных/autosave/restore ревизий и не применяется к baseline/create/delete/import/generator. Runtime actions не выполняют DDL; схему и совместимость со старым форматом применяют только служебные scripts.
