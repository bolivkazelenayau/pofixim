...
function extractNumberFromSeedKey(seedKey) {
  const match = String(seedKey ?? '').match(/ege11-bank-(\d+)$/);
  ...
  try {
    const rows = await sql`
      ...
      where type = 'ege_multi_select'
        and seed_key like 'ege11-bank-%'
      order by id asc
    `;

    const lines = ['# ЕГЭ-11 bank: номер, ID, правильный ответ и explanation', ''];
    let exportedCount = 0;
    ...
    const outPath = path.join(outDir, 'bank_ege11_number_id_answer_explanation.md');
