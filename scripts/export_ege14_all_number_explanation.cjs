  ...
  try {
    const rows = await sql`
      ...
      where type = 'ege_multi_select'
        and (seed_key like 'ege14-%' or exists (select 1 from unnest(skill_tags) t where t='ege.14'))
      order by ref_num asc nulls last, id asc
    `;

    const out = [];
    out.push('# ЕГЭ 14: номер и explanation');
    out.push('');
    ...
    const file = path.join(dir, 'ege14_all_number_explanation.md');
