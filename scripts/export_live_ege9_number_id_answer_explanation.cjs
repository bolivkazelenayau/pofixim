...
function extractNumberFromSeedKey(seedKey) {
  const match = String(seedKey ?? '').match(/live-ege9-(\d+)$/);
  ...
  try {
    const rows = await sql`
      ...
      where type = 'ege_multi_select'
        and seed_key like 'live-ege9-%'
      order by id asc
    `;

    const lines = ['# ЕГЭ-9: номер, ID, правильный ответ и explanation', ''];
    let exportedCount = 0;
    ...
    const outPath = path.join(outDir, 'live_ege9_number_id_answer_explanation.md');
