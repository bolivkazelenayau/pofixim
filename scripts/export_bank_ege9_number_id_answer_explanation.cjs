...
function extractNumberFromSeedKey(seedKey) {
  const match = String(seedKey ?? '').match(/ege9-bank-(\d+)$/);
...
function extractCorrectAnswer(answer) {
  ...
  if (typeof answer?.rawAnswerText === 'string' && answer.rawAnswerText.trim()) {
    return answer.rawAnswerText.trim();
  }
  ...
  try {
    const rows = await sql`
      ...
      where type = 'ege_multi_select'
        and seed_key like 'ege9-bank-%'
      order by id asc
    `;

    const lines = ['# ЕГЭ-9 bank: номер, ID, правильный ответ и explanation', ''];
    let exportedCount = 0;
    ...
    const outPath = path.join(outDir, 'bank_ege9_number_id_answer_explanation.md');
