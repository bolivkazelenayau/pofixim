...
const INPUT_FILES = [
  'C:/Users/Breeze/Downloads/live_ege11_number_id_answer_explanation_reformatted.md',
  'C:/Users/Breeze/Downloads/bank_ege11_number_id_answer_explanation_reformatted.md',
];
...
(async () => {
  ...
  try {
    const ege11Rows = await sql`
      select id, type, seed_key, skill_tags, explanation, source_alignment
      from exercises
      where type = 'ege_multi_select'
        and (
          seed_key like 'ege11-%'
          or exists (select 1 from unnest(skill_tags) s where s = 'ege.11')
        )
      order by id asc
    `;

    const byCurrentId = new Map(ege11Rows.map((row) => [row.id, row]));
    const byRef = new Map();
    for (const row of ege11Rows) {
    ...
    for (const item of parsedItems) {
      ...
      const isEge11 =
        String(row.seed_key ?? '').includes('ege11') ||
        (Array.isArray(row.skill_tags) && row.skill_tags.includes('ege.11'));
      if (row.type !== 'ege_multi_select') {
        throw new Error(`ID ${row.id} is type=${row.type}, expected ege_multi_select`);
      }
      if (!isEge11) {
        throw new Error(`ID ${row.id} is not marked as EGE-11`);
      }
    ...
    if (!resolvedUpdates.length) {
      throw new Error('No matching EGE-11 rows found in DB for the provided markdown files');
    }
    ...
    const verifyRows = await sql`
      select id, left(explanation, 120) as explanation_preview
      from exercises
