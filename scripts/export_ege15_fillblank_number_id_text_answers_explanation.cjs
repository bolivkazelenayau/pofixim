...
function extractNumber(row) {
  ...
  const seedMatch = seedKey.match(/(?:live-)?ege15(?:-bank)?-(\d+)$/);
...
(async () => {
  ...
  try {
    ...
    const out = [];
    out.push('# ЕГЭ 15 fill_blank: номер, ID, текст, правильные ответы и explanation');
    out.push('');
    ...
    const file = path.join(dir, 'ege15_fillblank_number_id_text_answers_explanation.md');
